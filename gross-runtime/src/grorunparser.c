			...
			while(TRUE) {
				...
				if (transition==NULL) {
					...
					if (transitions_for_symbol==NULL) {
						CatStringWo *s = cat_string_wo_new_with("");
						GroRunIToken *tt = grorun_stack_entry_get_lookahead(top_entry);
						GroRunSymbol *tt_sym = grorun_itoken_get_symbol(tt);
						cat_string_wo_format(s, "Syntax error: symbol '%O' not expected here", tt_sym);
...
//						context.markForkAsError(fork_idx, "no transitions for symbol:"+top_entry.lookahead+" for state:"+top_entry.state);
				...
				if (GRORUN_IS_REDUCE_TRANSITION(transition)) {
					grorun_fork_add_postponed(fork, grorun_stack_entry_get_lookahead(top_entry));
