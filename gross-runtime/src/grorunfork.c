...
GroRunFork *grorun_fork_new() {
	...
	priv->postponed = cat_array_wo_new();
	priv->push_back = NULL;
	priv->messages = NULL;
	priv->pushback_sub = 0;
	return result;
...
GroRunFork *grorun_fork_duplicate(GroRunFork *orig_fork, int recovery_count) {
	...
	priv->pending_transition = NULL;
	priv->postponed = cat_array_wo_clone(spriv->postponed, CAT_CLONE_DEPTH_MAIN);
	priv->push_back = cat_array_wo_clone(spriv->push_back, CAT_CLONE_DEPTH_MAIN);
...
gboolean grorun_fork_has_pushback(GroRunFork *fork) {
	GroRunForkPrivate *priv = grorun_fork_get_instance_private(fork);
	if (priv->push_back!=NULL) {
		priv->pushback_sub = 30;
		return TRUE;
	}
	if (priv->pushback_sub>0) {
		priv->pushback_sub--;
		return TRUE;
	}
	return FALSE;
...
void grorun_fork_dump(GroRunFork *fork) {
	...
	cat_log_print("FORK", " - push-back     : %O", priv->push_back);
